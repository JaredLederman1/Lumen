...
function computeAlerts(rows: EditRow[], income: number): Alert[] {
  ...
  if (Math.abs(gap) > 0.5) {
    if (gap > 0) {
      alerts.push({
        level: 'warning',
        message: `${fmt(gap)} is unallocated. Assign it to a category to balance your budget.`,
      })
    } else {
      alerts.push({
        level: 'error',
        message: `Over budget by ${fmt(Math.abs(gap))}. Reduce category amounts to balance.`,
      })
    }
  }
  ...
  if (!hasSavingRow) {
    alerts.push({
      level: 'error',
      message: 'No savings category. Add at least one savings allocation to build long-term wealth.',
    })
  } else if (income > 0 && savingTotal / income < 0.10) {
    const savingPct = ((savingTotal / income) * 100).toFixed(0)
    alerts.push({
      level: 'warning',
      message: `Savings is ${savingPct}% of income. A minimum of 10% is recommended. Consider increasing your savings allocation.`,
    })
  }
  ...
  if (hasDebtRow && income > 0 && debtTotal / income < 0.03) {
    alerts.push({
      level: 'info',
      message: 'Debt payments are under 3% of income. If you carry high-interest debt, allocating more accelerates payoff significantly.',
    })
  }
  ...
  return (
    ...
    >
      {(Object.keys(typeMeta) as CategoryType[]).map(t => (
        <option key={t} value={t} style={{ backgroundColor: '#0F1318', color: '#F0F2F8', textTransform: 'uppercase' }}>
          {typeMeta[t].label}
        </option>
      ))}
    </select>
...
function BudgetEditor({
  ...
  const alerts = computeAlerts(rows, monthlyIncome)
  const total = rows.reduce((s, r) => s + (r.amount || 0), 0)
  ...
  function addRow() {
    setRows(prev => [...prev, { _id: nextId(), name: '', amount: 0, type: 'need' }])
    setSaveStatus('idle')
  }
            ...
            {rows.map(row => (
              ...
              >
                <div style={{
                  ...
                  <TypeSelect
                    value={row.type}
                    onChange={v => updateRow(row._id, 'type', v)}
                  />
                  <InputField
                    type="number"
                    value={row.amount || ''}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        <button
          ...
          style={{
            ...
            border: 'none',
            borderRadius: '2px',
            color: saveStatus === 'saving' ? 'var(--color-gold)' : '#0F1318',
            fontFamily: 'var(--font-mono)',
            fontSize: '12px',
            letterSpacing: '0.08em',
            textTransform: 'uppercase',
...
export default function BudgetPage() {
  ...
  // Load existing saved budget on mount
  useEffect(() => {
    fetch('/api/budget')
      .then(r => r.json())
      .then(d => {
  ...
  async function runAnalysis() {
    ...
    try {
      const res = await fetch('/api/budget/recommend', { method: 'POST' })
        ...
        <div style={card}>
          ...
          <button
            ...
            style={{
              ...
              color: '#0F1318', fontFamily: 'var(--font-mono)', fontSize: '12px',
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
              ...
              <button
                ...
                style={{
                  ...
                  border: 'none', borderRadius: '2px', color: '#0F1318',
