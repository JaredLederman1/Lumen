...
export function crossCheckBenefits(e: ExtractedBenefits): BenefitStatus[] {
  ...
  // 401(k) match
  if (e.has401k) {
    const hasMatchData = e.matchRate != null && e.matchRate > 0 && e.matchCap != null && e.matchCap > 0
    const annual = hasMatchData ? Math.round(salary * e.matchCap! * e.matchRate!) : null
    ...
    results.push({
      ...
      evidence: hasRetirementAccount
        ...
        : 'No 401k account linked. Employer 401k enrollment not confirmed.',
      urgency: 'critical',
      action: hasMatchData
        ? `Contribute at least ${(e.matchCap! * 100).toFixed(0)}% of salary to capture the full ${fmt(annual!)}/yr employer match.`
        : 'Confirm 401k enrollment with HR and contribute enough to capture any available employer match.',
    })
  }
...
export function analyzeBenefitsGaps(benefits: LegacyBenefitsShape, annualIncome: number, yearsToRetirement: number): BenefitsGap[] {
  ...
  if (benefits.has401k) {
    const hasMatchData = benefits.matchRate != null && benefits.matchRate > 0 && benefits.matchCap != null && benefits.matchCap > 0
    const annual = hasMatchData ? annualIncome * benefits.matchCap! * benefits.matchRate! : 0
    const action = hasMatchData
      ? `Contribute at least ${(benefits.matchCap! * 100).toFixed(0)}% of salary to capture the full employer match of ${fmtLegacy(annual)} per year`
      : 'Confirm 401k enrollment with HR and contribute enough to capture any available employer match'
    gaps.push({ benefit: '401(k) Employer Match', offered: true, likelyCaptured: false, annualDollarValue: Math.round(annual), lifetimeDollarValue: fv(annual, yearsToRetirement), urgency: 'critical', action })
  }
